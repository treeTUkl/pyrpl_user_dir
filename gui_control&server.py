        ...
        while GUI.readQueuebool:
            ...
            if not GUI.standaclient is None:
                if GUI.Standa_Connected == True:
                    ...
                    if GUI.clientprintqueue.empty() == False:
                        clientstatus = GUI.clientprintqueue.get()
                        if clientstatus[0] == "printme":
                            string = str(clientstatus[1])
                            GUI.windowprintqueue.put(['printme', string])
                        elif clientstatus[0] == "Standa_Refused":
                            GUI.standa_live_control = False
                            GUI.Standa_Connected_check(False)
                            GUI.standaclient = False
                            GUI.windowprintqueue.put(['printme', "Standa Connection Refused"])
                            ...
                            if CurSpeed != "0" or uCurSpeed != "0" or MoveSts != "0":
                                ...
                                if GUI.run_messung is True:
                                    ...
                                    elif GUI.hold_messung is True:
                                        GUI.windowprintqueue.put(['printme', "Messung is paused"])
                                        GUI.windowprintqueue.put(['printme',
                                                                  "But Standa is still moving to next Messpoint and will w8 there"])
                                ...
                                if GUI.run_messung is True:
                                    if GUI.messung_pos == asPosition:
                                        ...
                                        if GUI.standa_moving_Check() is False:
                                            ...
                                            GUI.clientsendqueue.put(['Mess', str(GUI.messung_pos)])
                                            GUI.standa_moving_Check(True)
                                    GUI.windowprintqueue.put(['printme', "couse of State i check POS"])
                                    GUI.clientsendqueue.put(['POS', ""])
                            GUI.print_list.addItem('STATE: ' + clientstatus[1])
                    ...
                    if GUI.clientsendqueue.empty():
                        if GUI.clientsendqueue.empty():
                            now = time.time()
                            delta = now - timerstart
                            if GUI.standa_live_control is True:
                                w8time = 1
                            elif GUI.run_messung is True:
                                w8time = 3
                            else:
                                w8time = 10
                            if delta > w8time:
            ...
            if GUI.Standa_Connected is True:

                if GUI.is_alive():
                    counter = counter + 1
                    GUI.windowprintqueue.put(['printme', "standa still alive" + str(counter)])

                    time.sleep(2)
                else:
                    if counter > 0:
                        GUI.windowprintqueue.put(['printme', "standa died"])
                    counter = 0

            if GUI.windowprintqueue.empty() is False:
                while not GUI.windowprintqueue.empty():
                    ...
                    GUI.windowprintqueue.task_done()
            if GUI.Standa_Connected is False:
                GUI.Standa_Connected_check(False)
            time.sleep(0.01)
...
class Window(QtWidgets.QMainWindow):
    def _init_(self):
        ...
        self.clientsendqueue = queue.Queue()
        self.clientprintqueue = queue.Queue()
        self.messung_pos = 0
    ...
    def hold_Messung(self):
        if self.hold_messung is False:
            ...
            self.clientsendqueue.put(['STOPMOVE', ''])
    ...
    def Messung_Messpoint(self, aspos):
        ...
        if self.run_messung is True:
            if self.ac_messung:
                ...
                self.pyrpl_voltage = self.pyrpl_voltage / 20  # TODO WARUM /20 ? ist mittelwert von 20 abfragen
    ...
    def messung(self, chase=None):
        ...
        if chase is None:
            name = self.filePath_Edit.text()
            if name == "":
                QMessageBox.about(self, "Select Ac_File",
                                  "Ac_File is missing. Something went wrong please cancel manualy")
            else:
                ...
                if self.pyrpl_p is not None:
                    if self.pyrpl_Connected_check():
                        ...
                        if self.run_messung:
                            ...
                            self.clientsendqueue.put(['Mess', str(xitem)])
                            # time.sleep(0.2)
    ...
    def stop_messung(self):
        ...
        if self.Standa_Connected is True:
            self.clientsendqueue.put(['STOPMOVE', ''])
            self.clientsendqueue.put(['ClearList', ''])
    ...
    def Standa_Connect_to_Server(self):
        if self.standaclient is False:
            ...
            if standa_port == "":
                ...
                # while self.Standa_Connected is  False:
                self.windowprintqueue.put(
                    ['printme', "Connect to Standa_Server via:\n" + str(standa_ip) + ", " + str(standa_port)])
                # \*----Mutliprozess------*/
                # MP.set_start_method('spawn')
                #
                # self.standaclient= MP.Process(target=start_standaclient.client, args=(standa_ip, standa_port), daemon=True)
                clientsendqueue = queue.Queue()
                clientprintqueue = queue.Queue()
                self.standaclient = MP.Process(target=start_standaclient.client, args=(standa_ip, standa_port, clientprintqueue, clientsendqueue))
                # TODO/ from multiprocessing import Process, Queue standa need to get the queeues form als args!
                ...
                #     p.join()
                self.standaclient.start()
                time.sleep(2)
                result = self.standaclient.is_alive()
                if result is True:
                    self.Standa_Connected = True
                # \*----Original------*/
                # self.standaclient = start_standaclient.client(standa_ip, standa_port, GUI=self)
                if self.Standa_Connected is True:
                    self.windowprintqueue.put(['printme', 'Connection worked!'])
                    # now check wahts in standa!
                    self.standaclient.is_alive()
                    time.sleep(2)
                    self.standaclient.is_alive()
                    self.standaclient.is_alive()
                    self.standaclient.is_alive()
    ...
    def Standa_Close_Connection_to_Server(self):
        ...
        # \*----Mutliprozess------*/TODO
        self.standaclient.terminate()
        self.standaclient.join()
    ...
    def standa_handling(self):  # TODO this not needed? cause readQuee will handle
        if self.standa_live_control is False:
            self.standa_live_control_stop()
    ...
    def standa_move_relative(self):
        if self.standa_check():
            self.clientsendqueue.put(['MVRR', str(self.Pos_spinBox.value()) + ', ' + str(self.uPos_spinBox.value())])
            time.sleep(0.3)
    ...
    def standa_right(self):
        if self.standa_check() & self.standa_live_control:
            self.clientsendqueue.put(['RMOVE', ''])
            time.sleep(0.3)

    def standa_left(self):
        if self.standa_check() & self.standa_live_control:
            self.clientsendqueue.put(['LMOVE', ''])
            time.sleep(0.3)
    ...
    def standa_move_to(self):
        if self.standa_check() & self.standa_live_control:
            self.clientsendqueue.put(['MOVV', str(self.Pos_spinBox.value()) + ', ' + str(self.uPos_spinBox.value())])
            time.sleep(0.3)

    def standa_go_home(self):
        if self.standa_check() & self.standa_live_control:
            self.clientsendqueue.put(['MOVV', "0" + ', ' + "0"])
            time.sleep(0.3)

    def standa_set_home(self):
        if self.standa_check() & self.standa_live_control:
            self.clientsendqueue.put(['DEH', ''])
            time.sleep(0.3)
    ...
    def standa_set_settings(self):
        if self.standa_check() & self.standa_live_control:
            ...
            self.clientsendqueue.put(['MSET', send])
            time.sleep(0.03)
    ...
    def standa_moving_Check(self, bool=None):
        ...
        if bool is True:
            ...
            self.standa_moving_Check_radioButton.setStyleSheet(
                "QRadioButton::indicator {background-color:#32CC99;border: 2px solid white;}")
            ...
            self.standa_moving_Check_radioButton.setAutoFillBackground(True)
            self.standa_moving_Check_radioButton.setStyleSheet(
                "QRadioButton::indicator {background-color:white; border:2px solid white;}")
    ...
    def pyrpl_start(self):  # TODO: Info aus Console in Textbox übertragen
        ...
        # MP.set_start_method('spawn')
        #
        # self.pyrpl_p= MP.Process(target=start_pyrpl.pyrpl_p, args=(), daemon=True)
        # result = self.pyrpl_p.start()
        # result = self.pyrpl_p.is_alive()
